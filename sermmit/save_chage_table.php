...
$strSQL = "SELECT * FROM ordertype WHERE table_id = '".$_POST['txttable_id']."' ";
$objQuery = mysql_query($strSQL);
$objResult = mysql_fetch_array($objQuery);
if($objResult)
{
	echo "<script type='text/javascript'>alert('โต๊ะนี้ไม่ว่าง ');</script>";
	echo "<script>window.location='change_table.php'</script>";
}
else
{
	$strSQL = "UPDATE ordertype SET ";
	$strSQL .="table_id = '".$_POST["txttable_id"]."' ";
	$strSQL .="WHERE order_id = '".$_GET["orderID"]."' ";

$objQuery = mysql_query($strSQL);

		echo "<script type='text/javascript'>alert('ย้ายต๊ะเรียบร้อยแล้ว ');</script>";
		echo "<script>window.location='change_table.php'</script>";
}
